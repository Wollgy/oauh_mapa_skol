...
@section('content')
    ...
    <div class="row">
        <div class="col-md">
            <h2>{{ __('Úprava záznamu oboru') }}</h2>
        </div>
        <div class="col-md">
            <a class="btn btn-primary float-right" href="{{ route('obor.index') }}" title="Zpět na přehled">
    ...
    {{-- Edit form --}}
    <form action="{{ route('obor.update', $obor->id)}}" method="post">
        @csrf
        @method('put')
        ...
        <div class="form-group row">
            ...
            <div class="col-md-6">
                <input type="text" name="nazev" id="nazev" class="form-control" placeholder="{{ $obor->nazev }}" value="{{ $obor->nazev }}">
            </div>
        </div>
